...
{		
	...
	switch(SYS_CLK)
	{
		...
		case SYS_CLK_PLL:
			switchToPLL(SYS_PLL_SRC, PLL_IN_DIV, PLL_FB_DIV, PLL_OUT_DIV, 0);
			break;
		
		case SYS_CLK_PLL_DIV8:
			switchToPLL(SYS_PLL_SRC, PLL_IN_DIV, PLL_FB_DIV, PLL_OUT_DIV, 1);
			break;
...
// clksrc 可取值 SYS_CLK_24MHz、SYS_CLK_48MHz、SYS_CLK_XTAL
void switchToPLL(uint32_t clksrc, uint32_t indiv, uint32_t fbdiv, uint32_t outdiv, uint32_t div8)
{
	switchTo24MHz();
	
	if(clksrc == SYS_CLK_24MHz)
	{
		SYS->HRCCR = (1 << SYS_HRCCR_ON_Pos) |
					 (0 << SYS_HRCCR_DBL_Pos);		//HRC = 24Hz
